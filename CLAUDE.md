...
- HyperEVM token symbol decoding: `fetchTokenInfo` in `app/api/hyperswap/route.ts` uses `TextDecoder('utf-8')` for proper multi-byte char handling + `cleanSymbol()` to strip non-printable/non-ASCII chars. Fixes broken symbols like "USDa→0" showing up for tokens not in KNOWN_TOKENS.
...
- Wallet persistence on page refresh: set `autoConnect={true}` for both `WalletProvider` (Solana) and `SuiWalletProvider` (Sui). In Navbar, add useEffects to (1) persist `solanaAddress`/`suiAddress` to localStorage on change, and (2) restore from localStorage when the adapter auto-reconnects on page load — only restore if `adapter.publicKey.toBase58() === savedAddress` (security: never set from a locked wallet that has a stale address). EVM stays connected automatically via wagmi.
