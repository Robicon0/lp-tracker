...
- Hydration fix: `disabled={isFetching ? true : undefined}` causes SSR/client mismatch (server renders no attribute, client renders `disabled`). Use `disabled={isFetching}` — React handles boolean props natively.
