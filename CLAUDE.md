...
- **Manual snapshot route**: `app/api/snapshot/manual/route.ts` — POST `{address, chain}`. **No CRON_SECRET required** (browser can't carry server secrets). Snapshots a single wallet using the same `snapshotWallet` helper. The "Take Snapshot" button in analytics calls this route.
