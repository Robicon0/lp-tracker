...
- DefiLlama does NOT index ProjectX (and may not index other newer/smaller protocols) — APY correctly shows 0% for unindexed protocols. Do not fabricate APY. The `computePendingFees` silently returns {0n,0n} on any RPC failure — always add `console.error` logging at each failure point so Vercel function logs can diagnose issues.
