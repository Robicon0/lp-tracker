...
The analytics page computes portfolio-wide LP P&L entirely on-chain — there is **no snapshot cron, no database read, no "Take Snapshot" button, no historical chart**. The DB-backed snapshot system was removed; the `wallets` / `portfolio_snapshots` / `position_snapshots` tables and `scripts/setup-db.sql` are intentionally left in place in the repo but no code reads from or writes to them anymore. The orphaned `useDbPortfolioHistory` / `useDbPositionHistory` hooks are stubbed no-ops so the legacy `/dashboard/[id]` route still compiles.

- **Per-position P&L**: `app/lib/positionPnl.ts` `computePositionPnL` — pure function that takes an `ActivityEvent[]` from a protocol activity route plus current price0/price1 and returns `{initialValue, currentValue, feesCollected, feesUnclaimed, ilPct, ilUSD, netPnlUSD, netPnlPct, ...}` or `{ok:false, reason:'no_deposits'|'no_entry_prices'}`. The individual position detail page (`app/dashboard/position/[id]/page.tsx`) uses this for its on-chain P&L + IL card.
- **Aggregated LP P&L cards** (`app/analytics/page.tsx` SECTION 2.5): iterates every LP position in `useAllPositionsActivity`'s `eventsMap` and calls `computePositionPnL` per position. Sums include Total Initial Value, Current Value, Total Fees Collected, Total Fees Unclaimed, Total Impermanent Loss, Net P&L = `(Current + Fees Collected + Fees Unclaimed) − Initial`. Positions where `computePositionPnL` returns `{ok:false}` (or that have no price data) are excluded from the totals; the count is shown in a subtle "X positions excluded — entry data unavailable." note under the grid. Includes in-range, out-of-range, and closed positions. Lending/borrowing is intentionally excluded.
- **`useAllPositionsActivity`**: now returns `{perfMap, eventsMap, isLoading}`. `eventsMap` is `Map<positionId, ActivityEvent[]>` cached in localStorage (5min TTL), consumed by the analytics LP P&L aggregation. The closed-position filter was removed so P&L totals include closed positions.
- **Localstorage portfolio value chart** (`usePortfolioHistory.ts`) is unchanged — it still drives the "Portfolio Performance Chart" SECTION 2 on analytics. Only the DB-backed "Profit & Loss" section with the Take Snapshot button was removed.
...
- **Per-event entry prices on EVM activity routes**: all four EVM activity routes (`/api/aerodrome/activity`, `/api/uniswap/activity`, `/api/velodrome/activity`, `/api/hyperswap/activity`) now emit `price0AtTime` and `price1AtTime` on every ActivityEvent (nullable). These are the historical CoinGecko `/coins/{id}/history?date=DD-MM-YYYY` prices for the event's day, falling back to the caller-supplied current price. Consumed by the shared `computePositionPnL` helper in [app/lib/positionPnl.ts](app/lib/positionPnl.ts) for the position detail page's on-chain P&L + IL section.
...
- **Activity hooks fall back to current prices for `price0AtTime`/`price1AtTime` when no CoinGecko mapping exists** — this means for unknown tokens the entry prices equal current prices, which produces `ilPct = 0` mathematically. That is correct behavior: without historical prices we cannot compute IL, and showing 0% beats showing a fabricated number. The `no_entry_prices` branch in `computePositionPnL` only fires when EVERY priced-deposit weight is zero.
