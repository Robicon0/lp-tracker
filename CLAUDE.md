...
- **Snapshot logic**: extracted to `app/lib/snapshot.ts` (`snapshotWallet`, `baseUrlFromHeaders`) so both the cron route and the manual route share the same code path. `snapshotWallet` also UPSERTs into `wallets` so the row exists even if `/api/wallets/register` was never called.
- **Cron route**: `app/api/cron/snapshot/route.ts` — auth via `Authorization: Bearer ${CRON_SECRET}` (Vercel cron sends this automatically). Iterates rows in `wallets`, fetches positions for each via the existing `/api/{aerodrome,uniswap/v3,velodrome,hyperswap,pancakeswap,raydium,orca,cetus,bluefin,momentum}` routes (built absolute via `x-forwarded-host`/`x-forwarded-proto`), then writes one `portfolio_snapshots` row + one `position_snapshots` row per position. Supports `?wallet=0x...&chain=evm` for manual single-wallet snapshot. Both GET and POST accepted. **Always requires CRON_SECRET — do NOT call from the browser**, use `/api/snapshot/manual` instead.
- **Manual snapshot route**: `app/api/snapshot/manual/route.ts` — POST `{wallets:[{address,chain}, ...]}` (also accepts legacy `{address,chain}` for back-compat). **No CRON_SECRET required** (browser can't carry server secrets). Snapshots all wallets in parallel using the shared `snapshotWallet` helper. The "Take Snapshot" button in analytics now sends ALL connected wallets in one request (EVM + Solana + Sui) so a single click captures every chain including BNB/PancakeSwap. EVM routes scanned per wallet are defined in `app/lib/snapshot.ts` `EVM_ROUTES` (currently `aerodrome, uniswap/v3, velodrome, hyperswap, pancakeswap`); add new EVM protocol routes here.
- **History reads**: `app/api/history/portfolio?range=7d|30d|90d|all` (no wallet filter — sums every snapshot row in DB) groups rows whose timestamps are within 5 seconds of each other into one "batch" representing one Take Snapshot click across all wallets, then sums lp_value/fees/positionCount per batch. This is what makes the analytics chart show the true cross-wallet portfolio total even when wagmi is currently connected to a different EVM account than the one snapshotted previously. Optional `wallet=<addr>` or `wallets=<addr1,addr2>` query params still work for per-wallet history. `app/api/history/position?id=aero-50093212&range=...` is unchanged. Both return `{ ok, snapshots: [...] }` with timestamps in epoch ms.
...
- **Cron config**: `vercel.json` — `{ "crons": [{ "path": "/api/cron/snapshot", "schedule": "0 0 * * *" }] }` (daily at 00:00 UTC).
