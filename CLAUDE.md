...
- eth_getLogs RPC limits: Alchemy free tier caps at 10 blocks; Blast changed policy and now also caps at 10 blocks; mainnet.base.org caps at 10,000 blocks; DRPC free caps at 10,000 blocks. **LlamaRPC supports full-history eth_getLogs with no block-range limit** — use `https://base.llamarpc.com` for Base (Aerodrome), `https://op.llamarpc.com` for Optimism (Velodrome), `https://eth.llamarpc.com` / `https://arb1.llamarpc.com` / `https://polygon.llamarpc.com` for Uniswap chains. LlamaRPC is used for `eth_getLogs` in all activity routes; Alchemy is still used for `eth_getBlockByNumber` (timestamp lookups). Aerodrome CL NFT manager (`0x827922...`) was deployed at Base block ~13,844,000 (≈ April 2024). TokenId 50093212 has 7 Collect events (fee claims) + 1 IncreaseLiquidity starting at block 41,878,002.
