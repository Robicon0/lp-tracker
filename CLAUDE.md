...
- `useWalletTokens` scope — EVM only (by design, not a bug): The hook only reads `address` from `useAccount()` (EVM) and scans 5 Alchemy chains (Ethereum, Base, Arbitrum, Optimism, Polygon). It has zero Solana or Sui RPC calls. Tokens card = EVM ERC-20s only. Lending/Borrowing card = AAVE V3 aTokens/debtTokens only (detected by symbol prefix pattern `aBasUSDC`, `variableDebtEthWETH`, etc.). Solana/Sui token and lending support is NOT yet implemented — this is a known gap, not a regression.
- Hydration fix: The Refresh button `disabled` prop must be `disabled={mounted ? isFetching : false}` — `isFetching` is `undefined` during SSR but `true` on the client, causing a mismatch. Gating on `mounted` ensures both server and client render `disabled={false}` on the first pass. `mounted` state already exists at the top of the Dashboard component.
