...
// Pool Mint topic (same signature across all Uniswap-V3 forks) — identifies
// a position's OWN pool from its mint tx receipt.
...
export interface ActivityEvent {
  ...
  // per-position scan and the wallet-scope safety-net scan collapses to one
  // entry (per-position value wins). Mirrors the Aerodrome route.
  logIndex: number;
}
...
async function GET_impl(request: Request) {
  ...
  try {
    ...
    // resolved pool context, then merge. Replaces the previous
    // union-then-decode-with-one-context approach, which mis-scaled every
    // event belonging to a pool other than the single "representative" one
    // (x10^12 inflation or x10^-12 crushing, depending on which pool happened
    // to be open). Identical treatment to the Aerodrome route — Velodrome is
    ...
    if (walletScope) {
      ...
      for (const id of ids) {
        ...
        if (!ctx) {
          ...
          // with another pool's decimals — that is the bug itself.
          excluded.push({ tokenId: id, reason: 'pool-context-unresolved' });
          continue;
        }
        const sub = new URL('/api/velodrome/activity', origin);
      ...
      console.log(`[velodrome/activity] positionId=all account=${account} → ${ids.length} tokenIds, ${perPosition.length} resolved, ${excluded.length} excluded`);

      // Per-position breakdown so a WALLET-WIDE total can never be attributed
      // to a single position. Flat totals retained for compatibility and are
      // the sum of the per-position values by construction.
    ...
    const rawEvents: RawEvent[] = logs.flatMap((log) => {
      ...
      // On-chain log index — stable key for the analytics Fee Income dedup.
    ...
    const histPrices = pool && allBlocks.length > 0
      ? await (async () => {
          const resolver = createHistoricalFeePriceResolver({
            ...
            chain: 'optimism', // ITEM 0d — namespaces the Redis price key
    ...
    // archive miss no longer falls to current spot (Rule 1a — the cg-spot last
    // resort below is REMOVED for fee claims). Prewarm ONLY the (token, date)
    // pairs sqrtPriceX96 could NOT price, so common sqrtPrice-priced claims incur
    // zero DeFiLlama traffic. Non-stable sides only. Awaited so the synchronous
    // getCachedOnlyDefillamaPrice() read below finds it. Mirrors Sprint 1.15 Cetus.
    {
      ...
      if (dlNeed.size > 0) {
        await prewarmDefillamaPrices(
          [...dlNeed.entries()].map(([contract, tss]) => ({ chain: 'optimism' as const, contract, timestamps: [...tss] })),
        );
      }
    }
    ...
    const events: ActivityEvent[] = cleanRawEvents.map((ev) => {
      ...
      // NEVER current spot). Skipped when PRIORITY 1 (sqrtPriceX96) priced it.
      // Per side: stablecoin → $1; else CoinGecko historical (cache-only) THEN
      // DeFiLlama historical-by-contract (awaited-prewarmed above for
      // sqrtPrice-missed claims). If a side can't be priced historically the
      // claim stays UNRESOLVED (null) → "pending price resolution"; the prior
      // cg-spot last resort is REMOVED (Sprint 2.1b). Mirrors Sprint 1.15 Cetus.
      if (ev.type === 'fee_claim' && usdAtTime == null) {
        ...
        const priceSide = (tok: string, isStable: boolean, cgId: string | undefined): number | null => {
          ...
          const dl = tok ? getCachedOnlyDefillamaPrice('optimism', tok, ev.timestamp) : null;
