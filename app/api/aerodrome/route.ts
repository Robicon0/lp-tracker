...
export async function GET(request: Request) {
  ...
  try {
    ...
    // 3b. Discover unknown tokens (not in TOKENS) and enrich with on-chain
    // symbol/decimals + a CG symbol-search price lookup. Lets long-tail
    // Base altcoins surface in P&L instead of getting price=0 and being
    // excluded by computePositionPnL's missing_current_prices guard.
    ...
    const erc20Info: Record<string, Erc20Info> = unknownAddrs.length > 0
      ? await fetchErc20InfoBatch(BASE_RPC, unknownAddrs)
      : {};
    ...
    const positions = rawPositions.map((raw) => {
      ...
      // Get prices — try the hardcoded TOKENS path first, then fall back
      // to the CG symbol-search result keyed by lowercase address.
