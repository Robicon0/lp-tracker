...
|----------------------|-----------------|------------------|
| lp-pnl-events        | v28             | `87db23d` — Cetus V1 Add/RemoveLiquidityEvent parsed (pre-V2 deposits/withdrawals were invisible). Prior: Sprint MOMENTUM — Momentum is now an ACTIVITY_PROTOCOL (open positions route to `/api/momentum/activity` instead of being surfaced as unsupported rejections) AND its closed positions fold into Capital G/L; both change cached LP-P&L output. Parity with analytics-activity v19 |
| analytics-activity   | v20             | `87db23d` — Cetus V1 events enter Fee Income/activity, lockstep with lp-pnl-events v28. Prior: Sprint MOMENTUM — Momentum fee + historical-only reward claims now enter analytics Fee Income; re-resolve in lockstep with LP-P&L |
