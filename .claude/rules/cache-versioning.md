...
|----------------------|-----------------|------------------|
| lp-pnl-events        | v30             | ITEM 0g — Uniswap V3 and HyperSwap positions now carry a `poolAddress`, so their deposits/withdrawals are priced from the pool's historical `sqrtPriceX96` instead of a tick-boundary ESTIMATE. A v29 entry holds the old estimate-priced events and would keep feeding a wrong (and differently-marked) Capital G/L to returning users. Prior: ITEM 0b — activity events now carry `priceBasis`, and a deposit/withdrawal valued on a SUBSTITUTE basis (current spot, or the position's tick-boundary estimate) makes the position report as pricing-pending so Capital G/L declares itself incomplete. A v28 entry has no marker, so a cached substituted event would keep rendering as a settled historical value — the exact bug being fixed. Prior: `87db23d` — Cetus V1 Add/RemoveLiquidityEvent parsed (pre-V2 deposits/withdrawals were invisible). Prior: Sprint MOMENTUM — Momentum is now an ACTIVITY_PROTOCOL (open positions route to `/api/momentum/activity` instead of being surfaced as unsupported rejections) AND its closed positions fold into Capital G/L; both change cached LP-P&L output. Parity with analytics-activity v19 |
| analytics-activity   | v22             | ITEM 0g — lockstep with lp-pnl-events v30. Prior: ITEM 0b — lockstep with lp-pnl-events v29; cached events predate the `priceBasis` marker. Prior: `87db23d` — Cetus V1 events enter Fee Income/activity, lockstep with lp-pnl-events v28. Prior: Sprint MOMENTUM — Momentum fee + historical-only reward claims now enter analytics Fee Income; re-resolve in lockstep with LP-P&L |
| cetus-activity       | v5              | `87db23d` — Cetus V1 events in per-position activity. Prior: Sprint 2.2c — Cetus fee-claim SUI side historical-only via `getHistoricalOnlySuiPrice` (FIX-C spotFallback no longer reachable for fee claims; Rule 1a) |
...
| closed_pos_sui       | v2              | `87db23d` — Cetus V1 events change reconstructed Capital G/L; bluefin/momentum entries rebuild once (shared prefix, parsing unchanged). Prior: Sprint 2.2b — Redis cache of reconstructed CLOSED Sui positions' Capital G/L (key `closed_pos_sui_v1:{protocol}:{wallet}`, now `:cetus:`/`:bluefin:`/`:momentum:` namespaces, 30d TTL). NOT bumped for Sprint MOMENTUM — Momentum is a NEW protocol key, so cetus/bluefin entries are byte-identical. Bump on a closed-position valuation-logic change |
...
| vfat_sickles         | v1              | Sprint WRAPPER-PROTOCOLS (vfat/Sickle) — Redis cache of owner→Sickle resolution, key `vfat_sickles_v1:{owner}`. **Two TTLs by design:** a DEPLOYED Sickle address is immutable once CREATE2-created → 30 d; a **"no Sickle found" result → 5 min**, because that answer stops being true the moment the user opens their first vfat position, and a long TTL would hide the new position. A PARTIAL empty (at least one chain's RPC failed, `complete:false`) is NEVER cached — preserves the Sprint 1.14 intent that a transient failure must not freeze in. Caches a resolved result → versioned; bump on a resolution-semantics change. `lp-pnl-events`/`analytics-activity` NOT bumped (Sickle positions are new per-position entries keyed by their own id/URL; no existing cached entry changes shape — same reasoning as DefiTuna Phase 1) |
| evm_hist_price       | v1              | ITEM 0d — Redis cache of a pool's HISTORICAL `sqrtPriceX96`-derived USD prices at a finalized block (`evm_hist_price_v1:{chain}:{pool}:{blockHex}`), 90 d. A finalized block's pool price is IMMUTABLE, same contract as `evm_pos_ctx_v1`. **Only POSITIVE results are persisted** — a null may be a transient archive failure, and freezing it in would pin the position to a SUBSTITUTE price basis (tick estimate / current spot) for 90 days, i.e. permanently freeze the ITEM 0b bug. This is a DETERMINISM fix, not an optimization: keeping tier 1 warm is what stops the cascade silently changing basis between loads. Skipped entirely when the caller passes no `chain` (never guessed — the same pool address exists on several chains). `lp-pnl-events`/`analytics-activity` NOT bumped: the cached value is byte-identical to what a live archive read returns |
| evm_pos_ctx          | v1              | Sprint ITEM-A — Redis cache of a position's OWN pool + token context (`evm_pos_ctx_v1:{chain}:{nftManager}:{tokenId}`), the EVM analogue of `sui_pool_ctx_v1`. A position's pool and token pair are IMMUTABLE once minted → 90 d. **Only POSITIVE results are persisted**: a null may be a transient RPC failure, and freezing that in would permanently exclude a real position. Bump on a resolution-semantics change. `lp-pnl-events`/`analytics-activity` NOT bumped — this caches context, not a valued result, and per-position values were already correct |
